...
export default function About() {
    ...
    const imgRef = useReveal(200);
    const statsRef = useReveal(0);
    const expRef = useReveal(0);
    const eduRef = useReveal(0);
    const langRef = useReveal(0);
            ...
            <section className="section">
                <div className="section-inner">
                    ...
                    <div className="about-grid">
                        {/* Bio */}
                        <div className="about-text reveal" ref={bioRef}>
                            <p>
                        ...
                        {/* Portrait */}
                        <div className="about-image reveal" ref={imgRef}>
                            <img
                                src={gopikaImg}
                    ...
                    <div className="about-stats reveal" ref={statsRef}>
                        {STATS.map((s, i) => (
                            ...
                            >
                                <div className="metric-value" style={{ color: i % 2 === 0 ? '#09090b' : '#10b981' }}>
                                    {s.value}
                                </div>
                    ...
                    {/* Experience Timeline */}
                    <div style={{ marginTop: '5rem' }} className="reveal" ref={expRef}>
                        <p className="section-label">Professional Experience</p>
                        <div style={{ marginTop: '1.5rem' }}>
                            {TIMELINE.map((item, i) => (
                                <div key={i} className="timeline-row">
                                    <div>
                                        <div style={{ fontSize: '0.75rem', color: '#a1a1aa', letterSpacing: '0.04em' }}>{item.period}</div>
                                        <div style={{ fontSize: '0.82rem', color: '#10b981', marginTop: '0.3rem', fontWeight: 600 }}>{item.company}</div>
                                        <div style={{ fontSize: '0.75rem', color: '#a1a1aa', marginTop: '0.15rem' }}>{item.location}</div>
                                    </div>
                                    <div>
                                        <h3 style={{ fontWeight: 600, fontSize: '1rem', letterSpacing: '-0.01em' }}>{item.role}</h3>
                                        <ul style={{ marginTop: '0.75rem', paddingLeft: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                                            {item.bullets.map((b, j) => (
                                                <li key={j} style={{ fontSize: '0.875rem', color: '#52525b', lineHeight: 1.75, listStyleType: 'disc' }}>{b}</li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                    ...
                    {/* Education */}
                    <div style={{ marginTop: '5rem' }} className="reveal" ref={eduRef}>
                        <p className="section-label">Education</p>
                        <div style={{ marginTop: '1.5rem' }}>
                            {EDUCATION.map((item, i) => (
                                <div key={i} className="timeline-row">
                                    <div>
                                        <div style={{ fontSize: '0.75rem', color: '#a1a1aa', letterSpacing: '0.04em' }}>{item.year}</div>
                                        {item.score && <div style={{ fontSize: '0.75rem', color: '#10b981', marginTop: '0.25rem', fontWeight: 500 }}>{item.score}</div>}
                                    </div>
                                    <div>
                                        <h3 style={{ fontWeight: 600, fontSize: '1rem', letterSpacing: '-0.01em' }}>{item.institution}</h3>
                                        <p style={{ fontSize: '0.875rem', color: '#71717a', marginTop: '0.3rem', lineHeight: 1.6 }}>{item.degree}</p>
                                        <p style={{ fontSize: '0.75rem', color: '#a1a1aa', marginTop: '0.2rem' }}>{item.location}</p>
                                        {item.extra && <p style={{ fontSize: '0.75rem', color: '#a1a1aa', marginTop: '0.1rem' }}>{item.extra}</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Languages */}
                    <div style={{ marginTop: '5rem' }} className="reveal" ref={langRef}>
                        <p className="section-label">Languages</p>
                        <div style={{ display: 'flex', gap: '2rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
                            {['English', 'Malayalam'].map((lang, i) => (
                                <div
                                    key={lang}
                                    style={{
                                    ...
                                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#10b981'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(16,185,129,0.1)'; }}
                                    onMouseLeave={e => { e.currentTarget.style.borderColor = ''; e.currentTarget.style.boxShadow = ''; }}
                                >
                                    <span style={{ fontWeight: 600, fontSize: '1rem' }}>{lang}</span>
                                    <span style={{ fontSize: '0.75rem', color: '#a1a1aa', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Completely fluent</span>
                                </div>
                            ))}
                        </div>
                    </div>
